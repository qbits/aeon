...
(t/deftest test-intervals
  (let [d1 (datetime test-epoch)]
    ...
    (t/assert= (hour (add-interval d1 0 0 0 1 0 0)) 5)
    (t/assert= (hour (add d1 :hours 1)) 5)
...
(t/deftest test-diff-days
  (let [d (datetime test-epoch)]
    (t/assert= 0 (diff d d))
    (t/assert= -86400 (diff d (add-interval d 0 0 1 0 0 0)))))

(t/deftest test-value-eq
  (let [now (datetime)
        other-now (datetime (epoch now))]
    (t/assert= other-now now)))
